...
import (
	...
	tf "github.com/Myra-Security-GmbH/terraform-provider-myrasec/myrasec"
...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for MyraSec").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-jet-myrasec"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-jet-myrasec",
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add MyraSec APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, log, rl, setup, ws, pconfig.GetProvider(resourceMap), 1), "Cannot setup MyraSec controllers")
